...
class Maze:
    ...
    def filter_valid_waypoints(self):
        ...
        for i in range(3):
            for waypoint in self.__waypoints:
                new_alien = self.create_new_alien(waypoint[0], waypoint[1], i)
    ...
    def get_nearest_waypoints(self, cur_waypoint, cur_shape):
        ...
        """
        dis_and_loc = []
        valid_waypoints = self.__valid_waypoints.get(cur_shape, [])
        for waypoint in valid_waypoints:
            ...
            if self.is_valid_move((cur_waypoint[0], cur_waypoint[1], cur_shape), (waypoint[0], waypoint[1], cur_shape)):
                distance = euclidean_distance(cur_waypoint, waypoint)
                dis_and_loc.append((distance, waypoint))

        nearest_neighbors = [waypoint for _, waypoint in nsmallest(self.k, dis_and_loc)]
    ...
    def is_valid_move(self, start, end):
        ...
        if end_shape == -1 or end_shape == 3:
            return False
        self.move_cache[(start, end)] = False

        # same spot, diff shape
        new_alien = self.create_new_alien(start[0], start[1], end_shape)
        if start_pos == end_pos and does_alien_touch_wall(new_alien, self.walls):
            return False
        # diff spots, same shape
        if does_alien_path_touch_wall(new_alien, self.walls, end_pos):
            return False
        self.move_cache[(start, end)] = True
        return True
    ...
    def get_neighbors(self, x, y, shape_idx):
        ...
        # print(f"x y: {x} {y}, nearest:{nearest}")
        neighbors = [(*end, shape_idx) for end in nearest]
        #print(f"neighbors: {neighbors}")
        for end in [(x, y, shape_idx - 1), (x, y, shape_idx + 1)]:
            start = (x, y, shape_idx)
            # print(f"current:{start}, neighbor:{end}, is valid:{self.is_valid_move(start, end)}")
