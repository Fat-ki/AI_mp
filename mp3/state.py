...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for i in range(4):
            new_x = x + roww[i]
            new_y = y + coll[i]
            new_one = [s[:] for s in self.state]
            if 0 <= new_x < 3 and 0 <= new_y < 3:
#                print(f"new x, new y: {new_x}, {new_y}")
                new_one[x][y], new_one[new_x][new_y] = new_one[new_x][new_y], new_one[x][y]
#                print(f"new state: {new_one}")
    ...
    def compute_heuristic(self):
        ...
        for i in range(3):
            for j in range(3):
                cur_num = self.state[i][j]
                if cur_num == 0:
                    continue
                man = manhattan([i, j], [cur_num // 3, cur_num % 3])
#                print(f"num: {cur_num}, manhattan: {man}, i:{i}, j:{j}")
                total += man
        return total
