...
def training(sentences):
    ...
    tag_num_of_hapax = defaultdict(lambda: 0)

    for tag in tag_list:
        for word in emit_prob[tag]:
            if emit_prob[tag][word] == 1:
                tag_num_of_hapax[tag] += 1
    hapax_cnt = sum(tag_num_of_hapax.values())

    for tag in tag_list:
        total_emissions = sum(emit_prob[tag].values())
        vocab_size = len(emit_prob[tag])
        tmp = 1 if tag_num_of_hapax[tag] == 0 else tag_num_of_hapax[tag]
        alpha = tmp / hapax_cnt
        emit_prob[tag]["UNK"] = emit_epsilon * alpha / (
                total_emissions + emit_epsilon * (vocab_size + 1) * alpha)
        # print(emit_prob[tag]["UNK"])
        for word in emit_prob[tag]:
            emit_prob[tag][word] = (emit_prob[tag][word] + emit_epsilon * alpha) / (
                    total_emissions + emit_epsilon * (vocab_size + 1) * alpha)
